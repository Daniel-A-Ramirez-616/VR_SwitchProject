...
public class InputPoller : MonoBehaviour
{
    ...
    public InputData GetInputP0()
    {
        ...
        if (kb != null)
        {
            ...
            if(kb.eKey.isPressed) {input.buttonWest = true; }


            if (kb.upArrowKey.isPressed) { input.rightStick.y = 1; }
            if (kb.downArrowKey.isPressed) { input.rightStick.y = -1; }
            if (kb.leftArrowKey.isPressed) { input.rightStick.x = -1; }
            if (kb.rightArrowKey.isPressed) { input.rightStick.x = 1; }

            if (kb.iKey.isPressed) { input.dpad.y = 1; }
            if (kb.jKey.isPressed) { input.dpad.y = -1; }
            if (kb.kKey.isPressed) { input.dpad.x = -1; }
            if (kb.lKey.isPressed) { input.dpad.x = 1; }
